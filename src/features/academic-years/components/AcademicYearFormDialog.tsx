...
import { academicYearSchema, type AcademicYearFormData } from "../schemas/academic-year.schema"
...
export const AcademicYearFormDialog = ({ open, onClose, editingYear }: Props) => {
    ...
    const onSubmit = (data: AcademicYearFormData) => {
        if (isEditing && editingYear) {
            update(
                { id: editingYear.id, payload: data },
                { onSuccess: onClose }
            )
        } else {
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <div className="flex-1 min-w-0 pr-4">
                            <DialogTitle className="font-semibold text-base sm:text-lg">
                                {isEditing ? `Editar año ${editingYear?.name}` : 'Nuevo año académico'}
                            </DialogTitle>
                            ...
                            render={({ field, fieldState }) => (
                                <Field data-invalid={fieldState.invalid}>
                                    ...
                                    <InputWithIcon
                                        ...
                                        className="w-1/2"
                                    />
                                    <FieldDescription className="text-[11px] text-slate-400 dark:text-slate-500">4 dígitos numéricos. Ej: 2026</FieldDescription>
                                ...
                                <Field>
                                    <div className="flex items-center gap-3 py-1">
                                        <input
                                            ...
                                            className="w-4 h-4 accent-phoenix-gold cursor-pointer"
                                        />
                                        <FieldLabel htmlFor="ay-active" className="cursor-pointer mb-0">
                                            Marcar como año activo
                                        </FieldLabel>
                                    </div>
                                    <FieldDescription>
                                        El año activo es el contexto por defecto al iniciar sesión.
                                    </FieldDescription>
                                </Field>
                            )}
                        />
