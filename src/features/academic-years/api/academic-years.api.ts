...
export const academicYearsApi = {
  ...
  getAll: async (page = 1, perPage = 10): Promise<PaginatedResponse> => {
  ...
  create: async (payload: CreateAcademicYearRequest): Promise<AcademicYear> => {
    const { data } = await apiClient.post<{ data: AcademicYear }>(
      BASE,
      payload,
    );
    return data.data;
  ...
  update: async (
    ...
    const { data } = await apiClient.put<{ data: AcademicYear }>(
      `${BASE}/${id}`,
      payload,
    );
    return data.data;
