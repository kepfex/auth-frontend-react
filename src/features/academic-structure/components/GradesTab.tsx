...
import { useCreateGrade, useDeleteGrade, useGrades, useLevels, useUpdateGrade } from "../hooks/useAcademicStructure"
import type { Grade } from "../types/academic-structure.types"
import { Controller, useForm } from "react-hook-form"
import { gradeSchema, type GradeFormData } from "../schemas/academic-structure.schema"
...
export const GradesTab = () => {
    ...
    useEffect(() => {
        if (editing) {
            ...
            form.reset({ educational_level_id: levels[0]?.id ?? 0, code: '', name: '', order: 1 })
        }
    }, [editing, formOpen])

    const filteredGrades = filterLevel === 'all'
        ? grades
        ...
        <div className="space-y-4">
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {filteredGrades.map(grade => (
                            <tr key={grade.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                                <td className="px-4 py-3">
                                    <Badge variant="outline" className="text-xs">{grade.order}</Badge>
                                </td>
                                <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{grade.code}</td>
                                <td className="px-4 py-3 font-medium">{grade.name}</td>
                                <td className="px-4 py-3">
                                    <Badge variant="secondary" className="text-xs">
                                        {grade.educational_level?.name ?? levels.find(l => l.id === grade.educational_level_id)?.name ?? '—'}
                                    </Badge>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="flex justify-end gap-1">
                                        <Button size="icon" variant="ghost" className="h-7 w-7 hover:text-phoenix-gold"
                                            onClick={() => { setEditing(grade); setFormOpen(true) }}>
                        ...
                        {filteredGrades.length === 0 && !isLoading && (
                            <tr><td colSpan={5} className="text-center py-8 text-muted-foreground text-sm">Sin grados registrados</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            <Dialog open={formOpen} onOpenChange={(v) => { if (!v) { setFormOpen(false); setEditing(null) } }}>
                <DialogContent className="sm:max-w-sm">
                    <DialogHeader>
                        <DialogTitle>{editing ? `Editar ${editing.name}` : 'Nuevo grado'}</DialogTitle>
                        <DialogDescription>El código debe ser único en todo el sistema</DialogDescription>
                    </DialogHeader>
                    <form
                        id="grade-form"
                    ...
                    >
                        <FieldGroup>
                            <Controller name="educational_level_id" control={form.control} render={({ field, fieldState }) => (
                            ...
                            <Controller name="order" control={form.control} render={({ field, fieldState }) => (
                                <Field data-invalid={fieldState.invalid}>
                                    <FieldLabel htmlFor="gr-order">Orden dentro del nivel</FieldLabel>
                                    <Input {...field} 
                                    id="gr-order" 
                                    type="number"
                                     min={1} 
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" form="grade-form" disabled={isCreating || isUpdating}
                            className="bg-phoenix-gold hover:bg-phoenix-orange text-obsidian">
                            {isCreating || isUpdating ? 'Guardando...' : editing ? 'Guardar cambios' : 'Crear grado'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <AlertDialog open={!!deleting} onOpenChange={(v) => !v && setDeleting(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>¿Eliminar grado "{deleting?.name}"?</AlertDialogTitle>
                        <AlertDialogDescription>Se eliminará permanentemente. No se puede eliminar si tiene aulas habilitadas.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
